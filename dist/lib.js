...
export const intensityMap = ({ width, height }) => (intensityFn) => Array(width).fill(1).map((_, x) => Array(height).fill(1).map((_, y) => intensityFn(x, y)));
export const getRandomInt = (max) => {
    return Math.floor(Math.random() * Math.floor(max));
};
export const pick = (options) => options[getRandomInt(options.length)];
export const init = ({ width, height }) => () => intensityMap({ width, height })(() => undefined);
...
const render = (config) => (grid) => fillCanvas(createCanvas(grid, config), config, grid);
